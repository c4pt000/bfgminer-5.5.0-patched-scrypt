# bfgminer-5.5.0-patched-scrypt
bfgminer-5.5.0-patched-scrypt +DEB+RPM

```
autoreconf -fi &&  ./configure --disable-git-version --disable-adl --enable-scrypt && sudo make -j24 clean && sudo make -j24
make -j24 install
cp -rf /usr/local/bin/bfgminer /usr/bin/bfgminer
checkinstall --install=no --exclude=/sys/fs/selinux -D
alien --scripts --to-rpm bfgminer-bfgminer_5.5.0-patched-1_amd64.deb 
```
<br>
<br>
<br>
